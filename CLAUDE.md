...
- **수입(`D.income`)**: 앱테크처럼 생활비통장에 합쳐지는 돈. 카테고리 없음, `effectiveBudget()`이 `budgets[월] + 그 달 income 합계`로 계산해서 예산 게이지에 그대로 더해짐 — 지출 카테고리 분포(도넛)에는 포함 안 됨. 지출/수입 추가는 같은 바텀시트에서 상단 토글로 전환(`entryMode`), 목록도 필터 칩의 "수입"을 누르면 같은 리스트 컴포넌트가 지출 대신 수입 내역을 보여줌
